...
fn load_optional_entry_from_registry<T: winreg::types::FromRegValue>(key: &str) -> Result<Option<T>, Box<dyn std::error::Error>> {
    let reg_key = get_service_reg_key()?;
...
pub fn load_auth_token() -> Result<Option<String>, Box<dyn std::error::Error>> {
  ...
  Ok(load_optional_entry_from_registry(REGISTRY_AUTH_TOKEN_VALUE)?)
